import { createReferralService, getUserReferralsService, getReferralCountsService, getAllReferralsService, updateReferralStatusService, updateReferralService, deleteReferralService } from './referralService.js';
import { sendReferralEmail } from './emailService.js';
import { createCompatibleNotification } from '../notifications/notificationCompatibilityService.js';
...
export const createReferral = async (req, res, next) => {
  try {
    ...
    try {
      const { supabase } = await import('../../shared/config/supabase.js').then(m => ({ supabase: m.supabase }));
