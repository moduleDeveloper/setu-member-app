import { getUserHospitalMemberships } from '../../shared/utils/storageUtils.js';
...
const callOrderHistoryPurchaseRpc = async (request) => {
  ...
  const module = await import('../../shared/services/supabaseClient.js');
...
const enrichOrderRowsWithCatalog = async (rows, trustId) => {
  ...
  try {
    const { fetchTrustProductsCategories, buildProductCatalogLookups } = await import('./productCart.js');
